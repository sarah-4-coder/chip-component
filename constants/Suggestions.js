export const Suggestions = [
  { name: "John Doe", email: "john@example.com" },
  { name: "Jane Smith", email: "jane@example.com" },
  { name: "Michael Johnson", email: "michael@example.com" },
  { name: "Emily Brown", email: "emily@example.com" },
  { name: "David Wilson", email: "david@example.com" },
  { name: "Sarah Taylor", email: "sarah@example.com" },
  { name: "Christopher Lee", email: "christopher@example.com" },
  { name: "Amanda Martinez", email: "amanda@example.com" },
  { name: "Matthew Anderson", email: "matthew@example.com" },
  { name: "Jessica Garcia", email: "jessica@example.com" },
  { name: "Daniel Davis", email: "daniel@example.com" },
  { name: "Olivia Rodriguez", email: "olivia@example.com" },
  { name: "James Brown", email: "james@example.com" },
  { name: "Sophia Thomas", email: "sophia@example.com" },
  { name: "Alexander Hernandez", email: "alexander@example.com" },
  { name: "Emily Wilson", email: "emily@example.com" },
  { name: "William Moore", email: "william@example.com" },
  { name: "Madison Jackson", email: "madison@example.com" },
  { name: "Ethan White", email: "ethan@example.com" },
  { name: "Emma Taylor", email: "emma@example.com" },
];
